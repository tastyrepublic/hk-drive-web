...
import { 
  ...
  List, AlertCircle, UserCog, Clock, Plus 
} from 'lucide-react';
...
export function NextLessonCard({ upcomingLessons, onCancelLesson, onOpenHistory, onBookLesson, variants }: Props) {
    ...
    >
        <div className="flex flex-col flex-1 h-full">
            <div className="flex items-center justify-between mb-4 z-10 relative">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={onBookLesson} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors backdrop-blur-md border border-white/10" title={t('common.book_lesson', 'Book Lesson')}>
                ...
                <AnimatePresence initial={false} custom={direction} mode="wait">
                    {hasLessons ? (
                        ...
                        <motion.div 
                            ...
                            animate={{ opacity: 1 }} 
                            exit={{ opacity: 0 }}
                            className="flex flex-col items-center justify-center text-sm gap-4 py-8"
                        >
                            <div className="opacity-60 flex flex-col items-center">
                              <Calendar size={40} strokeWidth={1} />
                              <span className="mt-2">{t('next_lesson.no_lessons')}</span>
                            </div>
                            {/* [UPDATED] Dark Pill Style with Translation */}
                            <button 
                              onClick={onBookLesson}
                              className="flex items-center gap-2 text-xs font-bold bg-black/20 px-4 py-2 rounded-full backdrop-blur-md border border-white/10 uppercase transition-all active:scale-95 hover:bg-black/30 text-white shadow-sm"
                            >
                              <Plus size={14} />
                              {t('common.book_lesson', 'Book a Lesson')}
                            </button>
                        </motion.div>
                    )}
                </AnimatePresence>
