...
export function SettingsView({ profile, setProfile, onSave, isLoading }: Props) {
  ...
  const activeSubTab = path.includes('/payouts') ? 'payouts' : 
                       path.includes('/subscription') ? 'subscription' : 'general';
  ...
  // --- 2. INTERNAL PROTECTION (React Router Navigation) ---
  ...
  return (
    <>
      ...
      <UnsavedChangesBar 
        ...
        onDiscard={() => {
            setDraftProfile(profile); // Revert changes
            blocker.proceed?.();      // Allow navigation to continue
        }} 
      />

      <div className="flex flex-col md:flex-row gap-8 transition-all duration-300 relative z-0">
        <SettingsSidebar 
          activeTab={activeSubTab} 
          onTabChange={(tab) => navigate(`/dashboard/settings/${tab}`)} 
        />
